...
const ORGAN_PRESETS=[
 ...
 {id:'rivers',name:'RIVERS PIANO',detail:'ORGAN-LIKE · HOLLOW / MYSTERIOUS',bars:[5,3,6,1],cutoff:2400,env:{attack:.006,peak:.145,sustain:.078,decay:.18,release:.26},tones:[['sine',.5,.34,-2],['triangle',1,.46,0],['square',2,.085,4],['sine',3.1,.045,-5]]},
 {id:'metal',name:'METAL PIANO',detail:'PIANO · METALLIC / FM',bars:[3,8,7,5],cutoff:9800,env:{attack:.003,peak:.145,sustain:.012,decay:.34,release:.22},tones:[['sine',1,.48,0],['sine',2.01,.33,5],['sine',3.98,.19,-7],['sine',6.07,.11,2]]},
 {id:'electric',name:'ELECTRIC 73',detail:'E-PIANO · WARM BELL',bars:[5,8,3,6],cutoff:5200,env:{attack:.006,peak:.17,sustain:.035,decay:.82,release:.3},tones:[['sine',1,.5,-3],['sine',2,.29,3],['triangle',3,.12,0]]},
 {id:'vintageep',name:'VINTAGE E-PIANO',detail:'E-PIANO · WARM / ANALOG',bars:[6,7,3,2],cutoff:3900,env:{attack:.009,peak:.16,sustain:.045,decay:.7,release:.3},tones:[['triangle',1,.5,-2],['sine',2,.3,2],['sine',2.99,.13,-4],['sine',5.02,.045,3]]},
 {id:'synthpop',name:'SYNTH-POP PIANO',detail:'E-PIANO · 80s / CHORUSED',bars:[5,8,4,3],cutoff:5000,env:{attack:.004,peak:.155,sustain:.032,decay:.58,release:.24},tones:[['triangle',1,.43,-7],['triangle',1,.42,7],['sine',2,.22,0],['sine',4.02,.09,3]]},
 {id:'tines',name:'SILVER TINES',detail:'E-PIANO · BRIGHT / GLASS',bars:[4,8,6,3],cutoff:8600,env:{attack:.003,peak:.15,sustain:.018,decay:.48,release:.24},tones:[['sine',1,.45,-2],['sine',2,.31,4],['sine',5.03,.16,-5],['triangle',3,.08,2]]},
 {id:'toy',name:'TOY PIANO',detail:'PIANO · SMALL / PERCUSSIVE',bars:[2,7,8,2],cutoff:7600,env:{attack:.002,peak:.13,sustain:.008,decay:.22,release:.11},tones:[['triangle',1,.38,0],['square',2,.13,-4],['sine',3.03,.27,3],['sine',5.96,.12,0]]},
 {id:'cathedral',name:'CATHEDRAL',detail:'ORGAN · 16′ + 8′ / WIDE',bars:[8,6,7,5],cutoff:3100,env:{attack:.08,peak:.145,sustain:.115,decay:.16,release:.22},tones:[['sine',.5,.32,-4],['triangle',1,.43,4],['sine',2,.24,0],['sine',3,.09,0]]},
 {id:'gospel',name:'GOSPEL ROTOR',detail:'ORGAN · BRIGHT / FULL',bars:[8,8,7,4],cutoff:5600,tones:[['sine',.5,.15,-3],['triangle',1,.49,3],['sine',2,.29,-5],['square',3,.065,4]]},
 {id:'reed',name:'REED & AIR',detail:'ORGAN · BRIGHT / NARROW',bars:[3,8,6,1],cutoff:4400,tones:[['sawtooth',1,.23,-5],['triangle',1,.48,5],['square',2,.08,0]]},
 {id:'glass',name:'GLASS FLUTES',detail:'HYBRID · 4′ + 2′ / CLEAR',bars:[2,5,8,7],cutoff:6800,tones:[['sine',1,.38,0],['sine',2,.34,0],['sine',4,.18,0]]},
 {id:'choir',name:'ANALOG CHOIR',detail:'HYBRID · SLOW / HUMAN',bars:[6,5,4,3],cutoff:2900,env:{attack:.24,peak:.13,sustain:.11,decay:.3,release:.28},tones:[['triangle',1,.48,-7],['triangle',1,.43,7],['sine',2,.16,0]]},
 {id:'bassorgan',name:'BASS PEDALS',detail:'ORGAN · 16′ / DEEP',bars:[8,3,1,1],cutoff:1450,env:{attack:.012,peak:.19,sustain:.12,decay:.12,release:.16},tones:[['sine',.5,.44,0],['triangle',1,.48,-3],['sine',2,.08,2]]}
];
function loadOrganData(){try{return Object.assign({preset:'warm',octave:0,volume:72,reverb:28,take:[],takeBpm:null,loop:false,clip:null},JSON.parse(localStorage.getItem('grid01-organ-v1'))||{})}catch{return {preset:'warm',octave:0,volume:72,reverb:28,take:[],takeBpm:null,loop:false,clip:null}}}
...
function releaseOrganVoice(v,when=engine.ctx?.currentTime||0){if(!v||v.released)return;v.released=true;const release=v.release||.09;try{v.gain.gain.cancelScheduledValues(when);v.gain.gain.setTargetAtTime(0,when,release);v.oscs.forEach(o=>o.stop(when+Math.max(.35,release*6)))}catch{}setTimeout(()=>organVoices.delete(v),Math.max(500,release*7000))}
...
function organTone(note,time,duration=null,presetId=organData.preset){
 if(!engine.ctx)return null;const c=engine.ctx,p=ORGAN_PRESETS.find(x=>x.id===presetId)||ORGAN_PRESETS[0],env=Object.assign({attack:.018,peak:.16,sustain:.105,decay:.08,release:.09},p.env||{}),gain=c.createGain(),filter=c.createBiquadFilter(),dry=c.createGain(),wet=c.createGain(),oscs=[];filter.type='lowpass';filter.frequency.setValueAtTime(p.cutoff,time);filter.Q.value=.75;gain.connect(filter);filter.connect(dry).connect(engine.master);filter.connect(wet).connect(engine.effects.reverb.input);dry.gain.value=organData.volume/100*.68;wet.gain.value=organData.reverb/100*.34;gain.gain.setValueAtTime(0,time);gain.gain.linearRampToValueAtTime(env.peak,time+env.attack);gain.gain.setTargetAtTime(env.sustain,time+env.attack,env.decay);
 ...
 const voice={note,gain,oscs,released:false,release:env.release};organVoices.add(voice);if(duration!==null)releaseOrganVoice(voice,time+Math.max(.06,duration));return voice
}
function midiNote(midi){const names=['C','Db','D','Eb','E','F','Gb','G','Ab','A','Bb','B'];return names[midi%12]+(Math.floor(midi/12)-1)}
function noteMidi(note){const match=note.match(/^([A-G](?:b)?)(\d)$/);return match?NAMES.indexOf(match[1])+(+match[2]+1)*12:60}
const ORGAN_KEY_STEPS={z:0,s:1,x:2,d:3,c:4,v:5,g:6,b:7,h:8,n:9,j:10,m:11,q:12,'2':13,w:14,'3':15,e:16,r:17,'5':18,t:19,'6':20,y:21,'7':22,u:23,i:24};
function organBaseMidi(){return 48+Math.max(-2,Math.min(2,organData.octave||0))*12}
function organNoteForKey(key){const step=ORGAN_KEY_STEPS[key];return step===undefined?null:midiNote(organBaseMidi()+step)}
function renderOrganKeyboard(){const el=$('#organKeyboard'),start=organBaseMidi();el.innerHTML='';let whites=0;for(let midi=start;midi<=start+24;midi++){const note=midiNote(midi),black=note.includes('b'),key=Object.keys(ORGAN_KEY_STEPS).find(k=>ORGAN_KEY_STEPS[k]===midi-start),b=document.createElement('button');b.className='organ-key '+(black?'black':'white');b.dataset.note=note;b.setAttribute('aria-label',note);b.innerHTML='<span>'+note+'<small>'+(key?key.toUpperCase():'')+'</small></span>';if(black)b.style.left=(whites/15*100)+'%';else whites++;b.onpointerdown=e=>{e.preventDefault();b.setPointerCapture(e.pointerId);organNoteOn(note,b)};b.onpointerup=()=>organNoteOff(note);b.onlostpointercapture=()=>organNoteOff(note);el.append(b)}const out=$('#organOctaveOutput');if(out)out.textContent=midiNote(start)+' — '+midiNote(start+24);$('#organOctDown').disabled=(organData.octave||0)<=-2;$('#organOctUp').disabled=(organData.octave||0)>=2}
function renderOrganPresets(){const el=$('#organPresets');el.innerHTML='';ORGAN_PRESETS.forEach((p,i)=>{const b=document.createElement('button');b.className='organ-preset '+(p.id===organData.preset?'active':'');b.innerHTML='<i></i><span>'+p.name+'<small>'+p.detail+'</small></span>';b.onclick=()=>{stopOrganVoices();organData.preset=p.id;persistOrgan();renderOrgan();toast('REGISTER '+p.name)};el.append(b)});const p=organPreset();$('#organVoiceName').textContent=p.name;$('#organPresetNumber').textContent=String(ORGAN_PRESETS.indexOf(p)+1).padStart(2,'0');$('#organDrawbars').innerHTML=p.bars.map((h,i)=>'<i style="--h:'+(10+h*5)+'px;--bar:'+["#f17f3d","#f2eee4","#36b979","#3978f6"][i%4]+'"></i>').join('')}
function takeLength(){return organData.take.reduce((m,e)=>Math.max(m,e.start+e.duration),0)}
function renderOrganRoll(){const roll=$('#organRoll');roll.innerHTML='';if(!organData.take.length){roll.innerHTML='<div class="empty-take">PRESS RECORD, THEN PLAY</div>';$('#organTakeCount').textContent='EMPTY'}else{const len=Math.max(8,takeLength()),midis=organData.take.map(e=>noteMidi(e.note)),low=Math.min(...midis),high=Math.max(...midis),span=Math.max(12,high-low);organData.take.forEach(e=>{const d=document.createElement('i'),midi=noteMidi(e.note);d.className='take-note';d.title=e.note+' / '+e.duration.toFixed(2)+'s';d.style.left=(e.start/len*100)+'%';d.style.width=(e.duration/len*100)+'%';d.style.top=(8+(high-midi)/span*108)+'px';roll.append(d)});$('#organTakeCount').textContent=organData.take.length+' NOTES'}$('#organClipStatus').textContent=organData.clip?'MOUNTED / '+organData.clip.events.length+' NOTES':'NOT MOUNTED';$('#organClipStatus').classList.toggle('organ-clip-mounted',!!organData.clip)}
...
let scopeGain=1,scopeTrails=[],scopeMode=localStorage.getItem('grid01-scope-mode')||'eq',eqLevels=Array(24).fill(0),eqPeaks=Array(24).fill(0),scopeFrame=0;
function drawWaveScope(g,w,h,time){const mid=h/2,points=[];g.fillStyle='#e5e6e0';g.fillRect(0,0,w,h);g.strokeStyle='rgba(70,73,69,.13)';g.lineWidth=1;for(let x=0;x<=w;x+=w/8){g.beginPath();g.moveTo(x,0);g.lineTo(x,h);g.stroke()}for(let y=h/4;y<h;y+=h/4){g.beginPath();g.moveTo(0,y+.5);g.lineTo(w,y+.5);g.stroke()}let live=false;if(engine.analyser){const d=new Uint8Array(engine.analyser.fftSize);engine.analyser.getByteTimeDomainData(d);const raw=Array.from(d,v=>v-128),smooth=raw.map((_,i)=>{let sum=0,weight=0;for(let j=-3;j<=3;j++){const k=Math.max(0,Math.min(raw.length-1,i+j)),wj=4-Math.abs(j);sum+=raw[k]*wj;weight+=wj}return sum/weight});let peak=0;smooth.forEach(v=>peak=Math.max(peak,Math.abs(v)));if(peak>1.1){live=true;const target=Math.max(1.6,Math.min(20,31/peak));scopeGain+=(target-scopeGain)*(target>scopeGain?.32:.12);smooth.forEach((v,i)=>points.push([i/(smooth.length-1)*w,mid+Math.tanh(v*scopeGain/25)*(h*.39)]))}}if(!live){const energy=playing?1.08:.8,warp=1+Math.abs(state.deform||0)/36;for(let x=0;x<=w;x+=3){const edge=Math.sin(Math.PI*x/w),y=mid+edge*energy*(Math.sin(x*.052*warp+time*1.35)*16+Math.sin(x*.128-time*.8)*7+Math.sin(x*.019+time*.4)*5);points.push([x,y])}}scopeTrails.unshift(points.map(([x,y])=>[x,y]));if(scopeTrails.length>5)scopeTrails.pop();const colors=['#3978f6','#36b979','#f17f3d','#e5534b'];for(let echo=scopeTrails.length-1;echo>0;echo--){g.beginPath();scopeTrails[echo].forEach(([x,y],i)=>i?g.lineTo(x-echo*3,y+(echo%2?echo:-echo)*.7):g.moveTo(x-echo*3,y));g.globalAlpha=.09+(scopeTrails.length-echo)*.025;g.strokeStyle=colors[(echo-1)%colors.length];g.lineWidth=1.3+echo*.2;g.stroke()}g.globalAlpha=1;const path=()=>{g.beginPath();points.forEach(([x,y],i)=>i?g.lineTo(x,y):g.moveTo(x,y))},gradient=g.createLinearGradient(0,0,w,0);gradient.addColorStop(0,'#3978f6');gradient.addColorStop(.48,'#36b979');gradient.addColorStop(.78,'#f17f3d');gradient.addColorStop(1,'#e5534b');path();g.strokeStyle='rgba(57,120,246,.2)';g.lineWidth=7;g.stroke();path();g.strokeStyle=gradient;g.lineWidth=2;g.stroke()}
function drawEqScope(g,w,h,time){g.fillStyle='#151914';g.fillRect(0,0,w,h);const bars=eqLevels.length,d=new Uint8Array(engine.analyser?.frequencyBinCount||128);if(engine.analyser)engine.analyser.getByteFrequencyData(d);const live=engine.analyser&&d.some(v=>v>3),pad=7,labelH=11,gap=3,barW=(w-pad*2-gap*(bars-1))/bars,segments=9,areaH=h-labelH-5,segGap=2,segH=(areaH-segGap*(segments-1))/segments;for(let i=0;i<bars;i++){const pos=i/(bars-1),center=Math.floor(Math.pow(pos,1.72)*(d.length-1));let target=0;if(live){let sum=0,count=0;for(let j=-2;j<=2;j++){sum+=d[Math.max(0,Math.min(d.length-1,center+j))];count++}target=Math.min(1,sum/count/190)}else target=.09+Math.max(0,Math.sin(i*.72+time*.7))*.055+(playing?.05:0);eqLevels[i]+=(target-eqLevels[i])*(target>eqLevels[i]?.34:.055);eqPeaks[i]=Math.max(eqLevels[i],eqPeaks[i]-.009);const x=pad+i*(barW+gap);for(let s=0;s<segments;s++){const threshold=(s+1)/segments,y=areaH-s*(segH+segGap)-segH,on=eqLevels[i]>=threshold;color=s<5?'#38b977':s<7?'#efc75e':s<8?'#f17f3d':'#e5534b';g.fillStyle=on?color:'rgba(101,112,97,.16)';g.fillRect(x,y,barW,segH)}const py=areaH-Math.min(1,eqPeaks[i])*areaH;g.fillStyle=eqPeaks[i]>.82?'#e5534b':'#efc75e';g.globalAlpha=.9;g.fillRect(x,Math.max(1,py),barW,1);g.globalAlpha=1}g.fillStyle='#758071';g.font="7px 'DM Mono'";g.textAlign='center';[['63',1],['250',6],['1K',12],['4K',18],['16K',23]].forEach(([label,i])=>g.fillText(label,pad+i*(barW+gap)+barW/2,h-2))}
function renderScopeMode(){$$('[data-scope-mode]').forEach(b=>b.classList.toggle('active',b.dataset.scopeMode===scopeMode));$('#scope').setAttribute('aria-label',(scopeMode==='eq'?'Vintage spectrum equalizer':'Live audio waveform')+'. Click to switch mode')}
function setScopeMode(mode){scopeMode=mode==='wave'?'wave':'eq';localStorage.setItem('grid01-scope-mode',scopeMode);renderScopeMode();scopeTrails=[];toast(scopeMode==='eq'?'VINTAGE EQ':'COLOR WAVE')}
function toggleScopeMode(){setScopeMode(scopeMode==='eq'?'wave':'eq')}
function drawScope(now=performance.now()){requestAnimationFrame(drawScope);if(now-scopeFrame<38)return;scopeFrame=now;const canvas=$('#scope'),g=canvas.getContext('2d'),w=canvas.width,h=canvas.height;g.clearRect(0,0,w,h);scopeMode==='eq'?drawEqScope(g,w,h,now/1000):drawWaveScope(g,w,h,now/1000)}
function savePatch(){
